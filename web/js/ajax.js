// Make an ajax call to the given url, then (if the call was successful) call the Success Callback fn, otherwise,
// set an error message into the DOM element that has id 'errorId' (or if errorId was not supplied or invalid,
// alert the error message to the user/HTML coder).
function ajax(url, callBackSuccess, errorId) {
    var httpReq; // The httpReq Object is now local to function "ajaxCall" (not global)
    ...
    // Specify the function that you want to run once the ajax call has returned the data. Ajax calls are asyncrhonous
    // (non-blocking). So, after you specify the function, some time may pass before the function runs.
    httpReq.onreadystatechange = function () { // this is an anonymous function
