...
public class DbMods {
    ...
    public static StringData insert(StringData inputData, DbConn dbc) {
        ...
        if (errorMsgs.getCharacterCount() > 0) {  // at least one field has an error, don't go any further.
            ...
            String sql = "INSERT INTO web_user (first_name, last_name, user_email, user_password, membership_fee, birthday, user_role_id) "
                    + "values (?,?,?,?,?,?,?)";
            ...
            pStatement.setString(3, inputData.userEmail); // string type is simple
            pStatement.setString(4, inputData.userPassword);
            pStatement.setBigDecimal(5, ValidationUtils.decimalConversion(inputData.membershipFee));
            pStatement.setDate(6, ValidationUtils.dateConversion(inputData.birthday));
            pStatement.setInt(7, ValidationUtils.integerConversion(inputData.userRoleId));
     ...
     public static StringData update(StringData inputData, DbConn dbc) {
        ...
        if (errorMsgs.getCharacterCount() > 0) {  // at least one field has an error, don't go any further.
            ...
            String sql = "UPDATE web_user SET user_email=?, first_name=?, last_name=?, user_password=?, membership_fee=?, birthday=?, "+
            ...
            pStatement.setString(2, inputData.firstName);
            pStatement.setString(3, inputData.lastName);
            pStatement.setString(4, inputData.userPassword);
            pStatement.setBigDecimal(5, ValidationUtils.decimalConversion(inputData.membershipFee));
            pStatement.setDate(6, ValidationUtils.dateConversion(inputData.birthday));
            pStatement.setInt(7, ValidationUtils.integerConversion(inputData.userRoleId));
